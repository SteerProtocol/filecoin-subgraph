...
import { JobExecuted, JobRegistered, GasDeposited, GasWithdrawn, JobToggledByCreator } from "../../generated/DynamicJobs/DynamicJobs";
...
export function handleJobRegistered(event: JobRegistered): void {
    ...
    if (job == null) {
        ...
        if (vault) {
            ...
            if (leaderBoard) {
                leaderBoard.numStaticJobs = leaderBoard.numStaticJobs.plus(BigInt.fromI32(1));
            } else {
        ...
        for(let counter=0;counter<event.params.jobInfo.length;counter++) {
            jobInfo.push(
                event.params.jobInfo[counter].toHexString()
            );
        }

        const targetAddresses: Array<string> = [];

        for(let counter=0;counter<event.params.targetAddresses.length;counter++) {
            targetAddresses.push(
                event.params.targetAddresses[counter].toHexString()
            );
        }
